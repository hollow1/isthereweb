...
for (let [i,url] of config.urls.entries()) {
    ...
    try {
        await page.goto(url, {waitUntil: 'networkidle' + config.networkIdleLevel, timeout:5000}); // networkidle0 = wait until 0 connections left; networkidel2 = wait until 2 connections left.
    } catch(e) {
        console.log(e.name);
        continue;
    }
